...
int main(int argc, char *argv[])
{
    ...
    if ((validity_check(argc, goal = (argc > 1) ? argv[1] : " ") == FAIL) || (!(file = fopen(argv[2], "r"))))
        goto validity_error;
    printf("%d", argc);
    if (!(X = parse_matrix_from_stream(file)))
        goto free_and_exit; /* checks if X is null after parse_matrix_from_stream */

    if (!(A = getSimilarityMatrix(X)))
        goto free_and_exit; /* checks if A is null after getSimilarityMatrix */
    if (strcmp("sym", goal) == 0)
        print_matrix(A); /* print similarity matrix if goal="sym" */

    if (!(D = getDiagonalDegreeMatrix(A)))
        goto free_and_exit; /* checks if D is null after getDiagonalDegreeMatrix */
    if (strcmp("ddg", goal) == 0)
        print_matrix(D); /* print diagonal degree matrix if goal="ddg" */

    if (!(W = getNormalizedSimilarityMatrix(A, D)))
        goto free_and_exit; /* checks if W is null after getNormalizedSimilarityMatrix */
    if (strcmp("norm", goal) == 0)
        print_matrix(W); /* print normalized similarity matrix if goal="norm" */
